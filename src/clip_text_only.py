...
def predict_affordance_proba(model, sentence, object, image_path_list):
    ...
    sentence_embeddings = get_text_embeddings([sentence]*len(choices))
    ...
    pred_scores = torch.mean(sentence_embeddings @ text_embeddings.T, dim=0).tolist()
...
# with open(f"./results/predictions_clip.pkl", 'wb') as content:
