...
def predict_affordance_proba(model, sentence, object_name, img_path):
    ...
    for affordance in classes:
        template = f"consider {sentence}. Now, can human {affordance} the {object_name}? Accompanying this query is an image of the {object_name}. Note that the image may contain noise or variations in appearance. Given the textual description and the image, answer Yes or No whether the human can {affordance} the {object_name}."
        ...
        generated_ids = model.generate(**inputs, eos_token_id=exit_condition, bad_words_ids=bad_words_ids, max_new_tokens=20)
        generated_text = processor.batch_decode(generated_ids, skip_special_tokens=True)

        # print(generated_text)
        predicted_answer = generated_text[0].split('Assistant:')[1].strip()
...
# predicted_affordances = predict_affordance_proba(model, sentence, object)
    ...
    predicted_affordances = predict_affordance_proba(model, sentence, object_name, img_path)
